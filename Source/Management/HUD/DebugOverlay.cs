...
public partial class DebugOverlay : Control
{
		...
		try
		{
			...
			try
			{
				...
				if (DebugLinesCheckBox != null)
					DebugLinesCheckBox.ButtonPressed = RuntimeEngine.ShowDebug;

				if (MaxFPSSpinBox != null)
					MaxFPSSpinBox.Value = Godot.Engine.MaxFps;
	...
	{
		RuntimeEngine.SetShowDebug(toggled);
			...
			switch (strings[0])
			{
				...
				case "connect":
					...
					if (method.StartsWith('d')) //direct
					{
						...
						RuntimeEngine.Instance?.JoinServer(split[0], port);
					}
					else if (method.StartsWith('n')) //nat
					{
						RuntimeEngine.Instance?.JoinNatServer(strings[2]);
					}
					else if (method.StartsWith('r')) //relay
					{
						RuntimeEngine.Instance?.JoinNatServerRelay(strings[2]);
					}
					break;
				case "respawn":
					player?.Respawn();
				...
				case "fps":
					...
					{
						Logger.Log($"Max FPS is currently {Godot.Engine.MaxFps}");
	...
	public override void _Process(double delta)
	{
		...
		_statsTextStringBuilder.AppendLine($"{IntLabel} FPS: {string.Format(IntValue, Godot.Engine.GetFramesPerSecond())}");
		_statsTextStringBuilder.AppendLine($"{IntLabel} Physics TPS: {string.Format(IntValue, Godot.Engine.PhysicsTicksPerSecond)}");
		...
		_statsTextStringBuilder.AppendLine($"{IntLabel} Max FPS: {string.Format(IntValue, Godot.Engine.MaxFps)}");
		...
		{
			_statsTextStringBuilder.AppendLine($"{IntLabel} Local Peer ID: {string.Format(IntValue, multiplayerPeer.GetUniqueId())}");
			_statsTextStringBuilder.AppendLine($"{StringLabel} Peer Status: {string.Format(StringValue, multiplayerPeer.GetConnectionStatus().ToString())}");
		}
