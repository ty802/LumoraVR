...
// IPoseFilter that lerps tracked position/rotation toward incoming
...
public class PoseSmoother : Component, IPoseFilter
{
    ...
    public void ProcessPose(AvatarSocket slot, Slot space, ref float3 position, ref floatQ rotation, ref bool isTracking)
    {
        if (!isTracking)
        {
            _hasPrevious = false;
